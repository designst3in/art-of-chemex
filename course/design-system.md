...
Scaffold for *Art of Chemex* site typography. Tokens live in `theme.css` (`:root`). Three type roles — do not mix.

## Type roles (strict)

| Role | Face | Weight / axes | Color | Where |
|------|------|---------------|-------|-------|
| **1 · Small labels** | Silkscreen | **400 only** — never 700; 16px; no antialias | Coffee `#5C3A21` (active nav → turquoise) | **`h3`+**, sidebar links, small labels, `.hud` / `.meta` / REVIEW badges, inline `code` |
...
<div class="type-specimen">
  ...
  <p class="hud">SIDEBAR · REVIEW · META · H3+</p>
</div>
...
**h3 is not Flex** — Silkscreen 400 label chrome (same as sidebar / `.hud`). No `--wght-h3` / `--wdth-h3` / `--opsz-h3` required.
...
- [Design attributes](design-attributes.md) — workshop anatomy diagram
